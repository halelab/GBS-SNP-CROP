...
# Requirement 2: Trimmomatic v.0.33 (Bolger et al., 2014)
#############################################################
...
if ($dataType eq "PE") {
	...
	if ($adaptor ne '0') {
		system ( "java -jar $work_trimmomatic PE -phred$phred -threads $threads $outR1 $outR2 $trimoPER1OUT $trimoSER1OUT $trimoPER2OUT $trimoSER2OUT ILLUMINACLIP:$adaptor LEADING:$leading SLIDINGWINDOW:$sliding TRAILING:$trailing MINLEN:$minlen" );
	} elsif ($adaptor eq '0') {
...
############################
	...
	if ($adaptor ne '0') {
		system ( "java -jar $work_trimmomatic SE -phred$phred -threads $threads $outR1 $trimoSER1OUT ILLUMINACLIP:$adaptor LEADING:$leading SLIDINGWINDOW:$sliding TRAILING:$trailing MINLEN:$minlen" );
	} elsif ($adaptor eq '0') {
