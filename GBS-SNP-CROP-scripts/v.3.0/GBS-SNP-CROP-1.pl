...
###############################################################################################################################
# GBS-SNP-CROP, Step 1. For description, please see Melo et al. BMC Bioinformatics (2016) 17:29 DOI 10.1186/s12859-016-0879-y.
###############################################################################################################################
...
."-enz2 <enzyme 2 restriction site residue sequence> -t <threads>\n";
...
if ($dataType eq "PE") {
	print "Parsing Paired-End reads ...\nEven using multi-threads this process can take a while ... be patient!\n";
		...
		while (!eof($IN1) and !eof($IN2)) {
			...
			my $file2 = <$IN2>;
   			if ($i % 4 != 0) {
				...
				#if ( $R1read[0] =~ /^(@.*:N:0:)\w{0,10}$/ ) { # take care only alphabetic index up to 10 letters
				if ( $R1read[0] =~ /^(@.*:N:0:)*/ ) { # This seems more general
					$R1read[0] = $1;
				} else {
					$R1read[0] = $R1read[0];
				}
