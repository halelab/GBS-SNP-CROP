...
###########################################################################################################
##########################################################################################
# Requirement 1: BWA aligner (Li & Durbin, 2009) - script tested with v0.7.12
# Requirement 2: SAMtools (Li et al., 2009) - script tested with v1.7
# Requirement 3: cpan modules Getopt::Long, Parallel:ForkManager
##########################################################################################
...
my $H = "\n###########################################################\n"
	...
	."-F: SAMtools flags controlled by CAPS F (i.e. flag reads to exclude). Numeric. Default: 2308\n"
	."-f: SAMtools flags controlled by small f (i.e. required flags to keep reads). Numeric. Default: 0\n"
...
print "Producing the mpileup files to support variant discovery...\n";
foreach my $file (@files) {
	my $pid = $pm->start and next;
