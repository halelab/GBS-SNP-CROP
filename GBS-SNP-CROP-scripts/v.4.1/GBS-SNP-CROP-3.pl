...
#
# For help: perl GBS-SNP-CROP-3.pl help
###########################################################################################################
#########################################################
# Requirement: cpan module Getopt::Long
#########################################################
...
my $H = "\n###########################################################\n"
	...
	."-fq: FASTQ file name seed (convention: FileNameSeed.PE.R1parsed.merged.trimmed.fq.gz). String. Required\n\n";
...
#########################
print "\n#################################\n# GBS-SNP-CROP, Step 3, v.4.1\n#################################\n";
my $sttime = time;

# Creating the demultiplexed sub-directory
...
if ($dataType eq "PE") {
	...
	my $input1 = join ("","$fastq_seed",".PE.R1parsed.merged.trimmed.fq.gz");
	print "\nCreating genotype-specific FASTQ files from file $input1 ...\n";
	...
	my $input2 = join ("","$fastq_seed",".PE.R2parsed.merged.trimmed.fq.gz");
	print "\nCreating genotype-specific FASTQ files from file $input2 ...\n";
...
##################################
	...
	my $input1 = join ("","$fastq_seed",".SE.R1parsed.merged.trimmed.fq.gz");
	print "\nCreating genotype-specific FASTQ files from file $input1 ...\n";
...
print "\nPlease, find results in the 'demultiplexed' sub-directory.\n";
