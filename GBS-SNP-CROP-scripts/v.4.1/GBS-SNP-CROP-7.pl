...
#
# For help: perl GBS-SNP-CROP-7.pl help
###########################################################################################################
#########################################################
# Requirements: cpan modules Getopt::Long, List::Util, Parallel::ForkManager
#########################################################
...
my $H = "\n###########################################################\n"
	...
	."-indel: Turn on indel parsing. String. Default: Turned off. However, if indel parsing was done in Step 6, it should be turned on here.\n"
...
$minHomoDepth = 11;					$minHomoDepthOneAlt = 48;
...
#########################
print "\n###############################\n# GBS-SNP-CROP, Step 7, v.4.1\n###############################\n";
my $sttime = time;

# Creating the variants sub-directory
my $dir = "variants";
...
if ($indels) {
	...
	open my $DEST, ">", "$output" or die "Can't initialize $output output file";
	...
	while (<$VAR>) {
		...
		# Delete putative variants for which the reference matches neither the primary nor secondary allele
		if (($pop_one_var ne $ref) and ($pop_two_var ne $ref)) {
			next;
		}

		# Initial filters based on population allele frequencies
		# Delete monomorphic loci
		if ( $pop_two_count == 0 ) {
			next;
		}

		# Alternative allele strength: minimum proportion of non-primary allele reads that are the secondary allele
		...
		# Subsequent filters based on genotype-level allele frequencies
		my $row = "";
		my $homo_pri = 0;
		my $hets = 0;
		my $homo_alt = 0;
		...
		# Calculating population-level parameters for additional user-guided downstream variant filtering
		...
		# Identifying possible paralogous variants based on deviation from a 1:1 allele-specific ratio among heterozygotes (McKinney et al. 2016)
		my $N = 0;
		my $NA = 0;
		my $Z;
		$N += $_ for @HetDepth;
		...
		# Minimum genotype representation filter
	...
	print "\nYour genotyping matrix file $output was successfully created and can be found in the variants sub-directory. \n";

	print "GBS-SNP-CROP called $lc variants (SNPs + indels) in your population using the following parameters:\n"
	...
	."Minimum read depth for each allele required to call heterozygotes: $minHeteroDepth\n"
