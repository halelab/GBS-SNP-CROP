...
#########################
print "\n#################################\n# GBS-SNP-CROP, Step 6, v.4.1\n#################################\n\n";
my $pm = new Parallel::ForkManager($threads);
my $sttime = time;

# Creating the mpileup sub-directory
my $dir = "mpileup";
...
open my $CountList, ">", "CountFileList.txt" or die "Can't initialize CountFileList.txt file\n";
...
if ($indels) {
	print "GBS-SNP-CROP will now identify and call both SNPs and indels based on the alignment data.\n\n";
	print "Counting nucleotides and filtering monomorphic sites for all genotypes. Even using multi-threads, this process can take a while ... call a loved one!\n";
	...
	foreach my $file (@files) {
		...
		while (<$PILEUP>) {
			...
			# Count indels frequency and store in a hash of indel type and count
			my %Indel_cnt;
			...
			# Work on a nucleotide-specific string
			$algn =~ s/\$//g;
			$algn =~ s/\*//g;
	...
	print "DONE.\n";
	print "\nCreating a comprehensive master matrix, with genotype-specific alignment summaries for all putative variant positions ...\n";
...
#########################
	...
	print "GBS-SNP-CROP will now identify and call only SNPs from the alignment dataset.\n\n";
	print "Counting nucleotides and filtering monomorphic sites for all genotypes. Even using multi-threads, this process can take a while ... take a stroll!\n";
...
print "\nThe master matrix was successfully created. Please proceed with the next step (variant filtering and genotype calling).\n";
